...
{ 
    ...
    public function loadBetsAdd(data ):void
	 ...
	 { 
		...
		component.betsPanel.title=event.menu.selectedItem.name
	}
	...
	{  
		 _eventArrayC=new ArrayCollection(event.result as Array);
			...
			if(  _eventArrayC.length>0)
			{
				component.betViewStack.selectedIndex=1
			} 
			else
			{
				component.betViewStack.selectedIndex=2
			}
					
			component.betEventRepeater.dataProvider=_eventArrayC
			event.currentTarget.removeEventListener("result", resultEventBetData); 
	}
		...
		private function selectGroupsData(event:ResultEvent):void
 	    {    
 	     	 ...
 	     	 if(_checkData.validation(event.result)) 
	    	 {
	    	     ...
	    	     _betsAC=new ArrayCollection();
					...
					var resCor:String;
						    ...
						    component.betEventData.dataProvider= _betsAC;
	    	 	}
	    	 	{
		   ...
		   else
		   {
		    	...
		    	component.betViewStack.selectedIndex=3
	...
	public function addBet(arrayData:Array ):void
	{ 
         var betsDataProvider:ArrayCollection=component.betEventData.dataProvider;
       var data = betsDataProvider.getItemAt(arrayData[0]);
       var data2=data.databets;
       var int:int=arrayData[1];
		...
		if(len>14)
		{
			Alert("Maximum 14 events per ticket")
			return
		}
		...
		{
			Alert.show("One event per ticket")
		}
