...
Y.Global.on('offerTableClicked',
	function(e)
	{ 
		betSlipRecordSet.add(e.data);
	}
) 
...
betSlipRecordSet.after('add',
	...
	{ 
		...
		html+='</p>';
		Y.one('<?php echo $this->getHtmlIds('ticket',true);?>').append(html);
		calculateOdds() 
	}
)
...
Y.one('#resetBet').on('click',
	function(e)
	{
		Y.one('<?php echo $this->getHtmlIds('ticket',true);?>').setContent('');
